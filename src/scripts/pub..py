...
class ManipulatorController:
    def __init__(self):
        ...
        self.publisher = rospy.Publisher('manipulator_cmd', Twist, queue_size=10)
        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber('camera/rgb/image_raw', Image, self.image_callback)
        self.move_cmd = Twist()
        self.rate = rospy.Rate(10)
        rospy.loginfo("ManipulatorController initialized")
    ...
    def image_callback(self, data):
        ...
        if contours:
            ...
            if M['m00'] > 0:
                ...
                if abs(error_x) < 20:
                    rospy.loginfo("Object centered, stopping manipulator")
    ...
    def stop_manipulator(self):
        """Stop the manipulator when the object is centered."""
        self.move_cmd.linear.x = 0.0
        self.move_cmd.angular.z = 0.0
        self.publisher.publish(self.move_cmd)
        rospy.loginfo("Manipulator stopped. Picking up object.")
...
def create_publisher():
    """Create a ROS publisher for the manipulator_cmd topic."""
    return rospy.Publisher('manipulator_cmd', Twist, queue_size=10)
...
def main():
    """Main function to run the manipulator controller."""
    try:
        manipulator_controller = ManipulatorController()
        manipulator_controller.spin()
    except rospy.ROSInterruptException:
